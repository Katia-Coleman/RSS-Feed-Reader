//
//  IndividualFeedViewController.swift
//  RSS Feed Reader
//
//  Created by Katia Coleman on 3/4/22.
//

import UIKit

class IndividualFeedViewController: UIViewController {
    ...
    var feedFics: [fic] = []
    var indexOfFic: [Int] = []
    var nameOfFeed: String = ""
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        //set label as the name of the feed
        nameOfFeed = FollowingViewController.feedClicked
        feedTitle.text = nameOfFeed
        ...
        //get the indexes of the fics in the overall list of fics
        var i = 0
        for item in allFics {
            if item.feedName == nameOfFeed {
                feedFics.append(item)
                indexOfFic.append(i)
            }
            i += 1
        }
...
extension IndividualFeedViewController: TableViewCellDelegate {
    ...
    func clickStar(with isStarred: Bool, index indexPath: IndexPath) {
        if isStarred {
            feedFics[indexPath.row].starFilled = true
            allFics[indexOfFic[indexPath.row]].starFilled = true
        }
        else {
            feedFics[indexPath.row].starFilled = false
            allFics[indexOfFic[indexPath.row]].starFilled = false
            feedFics.remove(at: indexPath.row)
    ...
    func goToAo3(index indexPath: IndexPath) {
        UIApplication.shared.open(URL(string: feedFics[indexPath.row].link)!)
    }
